...
URL="http://" + baseurl + "trade/failed/?page=1&limit=100&order=pubdate&dir=desc#table"
...
inf_file=open("./" + baseurl + str(datetime.now()) +"_roseltorg_failed_auctions.txt","w")
cur_page=1
while cur_page <= num_of_pages:
        print(str(cur_page) + '<=' + str(num_of_pages))
        if cur_page > 1:
                URL="http://" + baseurl + "trade/failed/?page="+('%d' % cur_page)+"&limit=100&order=pubdate&dir=desc#table"
                soup=BeautifulSoup(urllib.request.urlopen(URL),from_encoding="utf-8")
